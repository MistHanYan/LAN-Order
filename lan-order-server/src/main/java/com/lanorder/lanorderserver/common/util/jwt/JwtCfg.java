package com.lanorder.lanorderserver.common.util.jwt;
...
public class JwtCfg {
    ...
    public static String getToken(String username, int time) {
        Date now = new Date();
    ...
    public static boolean checkJwtTimed(String token){
        SecretKey key = Keys.hmacShaKeyFor(SECRET_KEY.getBytes());
        try {
            Jwts.parserBuilder()
                    .setSigningKey(key)
